    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="rounded-[2rem] border-none shadow-xl bg-primary/5 dark:bg-primary/10">
                    ...
                    <CardContent>
                        <div className="flex items-center gap-4">
                            ...
                            <span className="text-4xl font-black text-foreground">{warehouses?.length || 0}</span>
                        </div>
                    </CardContent>
                </Card>

                <Card className="rounded-[2rem] border-none shadow-xl bg-destructive/5 dark:bg-destructive/10">
                    <CardHeader className="pb-2">
                ...
                <Card className="rounded-[2rem] border-none shadow-xl bg-green-500/5 dark:bg-green-500/10">
                    <CardHeader className="pb-2">
                    ...
                    <CardContent>
                        <div className="flex items-center gap-4">
                            ...
                            <span className="text-4xl font-black text-foreground">{products?.length || 0}</span>
                        </div>
                    </CardContent>
                </Card>
            </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid gap-4">
                        {lowStockAlerts.map(alert => (
                            <Alert key={alert.id} variant="destructive" className="rounded-2xl border-none shadow-lg bg-destructive/10 dark:bg-destructive/20 text-foreground">
            ...
            <div className="bg-card dark:bg-black rounded-[2.5rem] border border-border dark:border-white/5 overflow-hidden shadow-2xl">
                <div className="p-8 border-b border-border dark:border-white/5 bg-secondary/5 dark:bg-white/2 flex justify-between items-center">
                    <div>
                        <h2 className="text-2xl font-black tracking-tighter text-foreground">Inventory Distribution</h2>
                    ...
                    <Dialog open={isWarehouseDialogOpen} onOpenChange={setIsWarehouseDialogOpen}>
                        ...
                        <DialogContent className="sm:max-w-[425px] rounded-[2rem] bg-background dark:bg-black border-border dark:border-white/5">
                            <DialogHeader>
                                <DialogTitle className="text-2xl font-black tracking-tighter text-foreground">Deploy New Warehouse</DialogTitle>
                            </DialogHeader>
                            <div className="grid gap-6 py-4">
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        className="bg-secondary/10 dark:bg-white/5 border-none h-12 rounded-xl font-bold text-foreground"
                ...
                <div className="flex flex-col md:flex-row min-h-[500px]">
                    {/* Product List */}
                    <div className="w-full md:w-1/3 border-r border-border dark:border-white/5 overflow-y-auto max-h-[600px]">
                        {products?.map(product => (
                            <button
                                key={product.id}
                                onClick={() => setSelectedProduct(product.id)}
                                className={`w-full text-left p-6 hover:bg-secondary/5 dark:hover:bg-white/2 transition-all flex items-center gap-4 border-b border-border dark:border-white/5 ${selectedProduct === product.id ? 'bg-primary/5 dark:bg-primary/10 border-r-4 border-r-primary' : ''}`}
                            >
                                <img src={product.image} className="w-12 h-12 rounded-xl object-cover" alt="" />
                                <div>
                                    <p className="font-black tracking-tight text-foreground">{product.name}</p>
                    ...
                    <div className="flex-1 p-8 bg-secondary/2 dark:bg-black/20">
                        {selectedProduct ? (
                            <div className="space-y-8 animate-in fade-in duration-300">
                                ...
                                <div className="grid gap-6">
                                    {warehouses?.map(warehouse => {
                                        ...
                                        return (
                                            <div key={warehouse.id} className="bg-card dark:bg-black border border-border dark:border-white/10 rounded-2xl p-6 flex items-center justify-between shadow-sm">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-12 h-12 rounded-full bg-secondary/10 dark:bg-white/5 flex items-center justify-center">
                                                        <WarehouseIcon className="w-6 h-6 text-muted-foreground" />
                                                    </div>
                                                    <div>
                                                        <p className="font-black text-lg text-foreground">{warehouse.name}</p>
                                                ...
                                                <div className="flex items-center gap-6">
                                                    <div className="text-right">
                                                        <p className="font-black text-2xl tracking-tighter text-foreground">{currentStock}</p>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <Input
                                                            type="number"
                                                            className="w-24 h-12 rounded-xl font-bold border-2 text-center text-foreground dark:border-white/10"
                                                            placeholder="New"
                            ...
                            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-50">
                                ...
                                <div>
                                    <h3 className="text-2xl font-black tracking-tighter text-foreground">Manifest Logistics</h3>
