                ...
                <div className="flex justify-between items-center bg-secondary/5 dark:bg-white/2 p-10 rounded-[2.5rem] border border-border dark:border-white/5">
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <h1 className="text-4xl font-black tracking-tighter text-foreground">Reverse Logistics</h1>
                        </div>
                        <p className="text-muted-foreground font-medium text-lg">Manage asset returns and exchange manifestos</p>
                    </div>
                    <div className="bg-background dark:bg-black px-6 py-4 rounded-2xl border border-border dark:border-white/10 shadow-xl">
                        <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground block mb-1">Queue Status</span>
                        <span className="text-2xl font-black text-foreground">{returns?.filter(r => r.status === 'pending').length || 0} Pending</span>
                    </div>
                </div>

                <div className="grid gap-6">
                    {!returns || returns.length === 0 ? (
                        <div className="text-center py-32 bg-secondary/5 dark:bg-white/2 rounded-[3rem] border-2 border-dashed border-border dark:border-white/10">
                        ...
                        returns.map((ret) => (
                            <div key={ret.id} className="group relative bg-card dark:bg-black border border-border dark:border-white/10 rounded-[2.5rem] overflow-hidden transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 border-l-8 border-l-primary/30">
                                <div className="p-10 flex flex-col lg:flex-row justify-between gap-10">
                                    <div className="space-y-6 flex-grow">
                                        <div className="flex flex-wrap items-center gap-4">
                                            <Badge
                                                className={`px-4 py-1 rounded-lg font-black uppercase text-[10px] tracking-widest border-none ${ret.status === 'pending' ? 'bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-500' :
                                                    ret.status === 'approved' ? 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-500' :
                                                        ret.status === 'completed' ? 'bg-primary/10 dark:bg-primary/20 text-primary' :
                                                            'bg-destructive/10 text-destructive'
                                                    }`}
                                            >
                                                {ret.status}
                                            </Badge>
                                            <div className="flex items-center gap-2 text-muted-foreground bg-secondary/5 dark:bg-white/5 px-3 py-1.5 rounded-xl border border-border dark:border-white/5">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="bg-secondary/5 dark:bg-white/2 p-6 rounded-2xl border border-border dark:border-white/5 italic text-lg font-medium text-foreground leading-relaxed">
                                                &quot;{ret.reason}&quot;
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-8 pt-2">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-xl font-black font-mono text-foreground">{formatCurrency(ret.order.total)}</span>
                                            </div>
                                            <div className="w-px h-10 bg-border dark:bg-white/10" />
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-xl bg-secondary/10 dark:bg-white/5 flex items-center justify-center">
                                                    <UserIcon className="w-5 h-5 opacity-50 text-foreground" />
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Originator</span>
                                                    <span className="text-sm font-bold text-foreground">{ret.order.customerFullName}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>

                                    <div className="flex flex-col gap-3 justify-center min-w-[240px] bg-secondary/5 dark:bg-white/2 p-6 lg:p-8 rounded-[2rem] border border-border dark:border-white/10">
                                        {ret.status === 'pending' && (
                                            <>
                                                <Button
                                                ...
                                                <Button
                                                    variant="outline"
                                                    className="w-full h-14 rounded-xl font-black uppercase text-xs tracking-[0.2em] text-destructive hover:bg-destructive/5 border-2 border-destructive/10 dark:border-destructive/20 transition-all font-bold"
                                        ...
                                        {ret.status === 'completed' && (
                                            <div className="text-center py-4">
                                                ...
                                                <p className="text-[10px] font-black uppercase tracking-widest text-emerald-600/60 font-bold">Strategy Finalized</p>
                                            </div>
                                        )}
                                        {ret.status === 'rejected' && (
                                            <div className="text-center py-4">
                                                <XCircle className="w-10 h-10 text-destructive mx-auto mb-2 opacity-30" />
                                                <p className="text-[10px] font-black uppercase tracking-widest text-destructive/40 font-bold">Request Terminated</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
